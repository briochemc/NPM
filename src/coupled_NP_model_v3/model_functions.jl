...
function wPOP(p) # does not depend on p
    @unpack w₀P, w′P = p
    return @. w₀P + w′P * z # same as POP
end
T_POP(p) = transportoperator(grd, w=wPOP(p))
function wPON(p) # does not depend on p
    @unpack w₀N, w′N = p
    return @. w₀N + w′N * z # same as POP
end
T_PON(p) = transportoperator(grd, w=wPON(p))
